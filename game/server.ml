...
let startGameServer port guiPort =
  ...
  let rec acceptPlayers () =
    let _ = try
      ...
      match Connection.input client with
        Some(Control(GameRequest)) -> performTSRefOp (fun l -> players := (!players)@[client]) players playersMutex;
                                      print_endline "A client has connected."
      | _ -> Connection.close client
    with e -> print_endline ("Listening for a client connection resulted in exception : " ^ Printexc.to_string e) in
    acceptPlayers()
  in
  ...
  let rec startNewGame () =
    ...
    let (game_temp,first_pick, atks, mons) = Game.init_game () in
	...
	let first_pick_request =  Request(PickRequest(first_pick,game ,atks, mons)) in
	let redRequest = if first_pick = Red then ref( Some first_pick_request) else ref None in
	let blueRequest = if first_pick = Blue then ref(Some first_pick_request ) else ref None in
       ...
       match (Mutex.lock msgMutex; !nextMsg) with
        ...
        | Some msg ->
            ...
            cont := match Connection.input client with
                | Some(Action(r)) -> myAction := (Action(r));
									true
	...
	let rec handleStep () =
		...
		let (result, new_state, red_request, blue_request) = Game.handle_step (Game.game_from_data(!game_state)) !redAction !blueAction in
	...
	match result with
      ...
      | Some c -> Netgraphics.add_update(Message("Winner:"^(string_of_gr c))); Netgraphics.send_updates();
...
let () =
  ...
  let guiPort = try int_of_string ((Sys.argv).(2))
                with _ -> cDEFAULT_GUI_PORT_NUMBER in let _ = print_endline ("GUI port: " ^ (string_of_int cDEFAULT_GUI_PORT_NUMBER)) in
